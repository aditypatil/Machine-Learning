...
class CategoricalFeatures:
    ...
    def transform(self, dataframe):
        ...
        if self.enc_type == "label":
            ...
            # dataframe = pd.get_dummies(dataframe, columns=self.cat_feats, drop_first=False)
            # # Ensure new data has same columns as training data
            # missing_cols = set(self.one_hot_encoders) - set(dataframe.columns)
            # for col in missing_cols:
            #     dataframe[col] = 0  # Add missing columns
            # return dataframe[self.one_hot_encoders]  # Keep only expected columns
        elif self.enc_type == "auto":
            if self.one_hot_cols:
                ...
                # dataframe = pd.get_dummies(dataframe, columns=self.one_hot_cols, drop_first=False)
                # missing_cols = set(self.one_hot_encoders) - set(dataframe.columns)
                # for col in missing_cols:
                #     dataframe[col] = 0  # Add missing columns
                # new_oh_cols = list(set(self.one_hot_encoders) - set(self.one_hot_cols))
                # for col in new_oh_cols:
                #     if dataframe[col].dtype == 'bool':  
                #         dataframe[col] = dataframe[col].astype(int)
                # dataframe = dataframe[self.one_hot_encoders]  # Ensure same column order
            if self.binary_cols:
